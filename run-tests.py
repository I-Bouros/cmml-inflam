...
def run_unit_tests():
    ...
    """
    tests = os.path.join('cmmlinflam', 'tests')
...
def run_copyright_checks():
    ...
    # Recursively walk the cmmlinflam directory and check copyright header is
    # in each checked file type
    header_check = True
...
#"""
    ...
    for dirname, subdir_list, file_list in os.walk('cmmlinflam'):
...
def doctest_rst_and_public_interface():
    ...
    # Import all public cmmlinflam modules. We list
    # everything here for completeness. If a new module is added to cmmlinflam
    ...
    check_exposed_symbols(cmmlinflam, cmmlinflam_submodules, doc_symbols)
...
def check_exposed_symbols(module, submodule_names, doc_symbols):
    ...
    if len(unexpected_modules) > 0:
        ...
        print('For python modules such as numpy you may need to confine the '
              ...
              'cmmlinflam submodule, you will need to make %s (doctest) aware'
              'of this.'
              % __file__)
        print('FAILED')
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run unit test for cmmlinflam',
        epilog='To run individual unit tests, use e.g.'
               ' $ python3 cmmlinflam/tests/test_dummy.py',
    )
    # Unit tests
    parser.add_argument(
        '--unit',
